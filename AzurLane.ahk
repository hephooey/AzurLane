...
iniread, ClearingMode, %SettingName%, Battle, ClearingMode
Gui, Add, CheckBox, x310 y%Tab1_Y% w80 h20 gAnchorsettings vClearingMode checked%ClearingMode% , 周回模式
...
Tab_Y -= 3
Gui, Add, Slider, x140 y%TAB_Y% w80 h30 gOthersettings vValue_Pic range0-500 +ToolTip , %Value_Pic%
Tab_Y += 3
...
Global Assault, Autobattle, shipsfull, ChooseParty1, ChooseParty2, AnchorMode, SwitchPartyAtFirstTime, ClearingMode, AnchorChapter, AnchorChapter2, mood
...
Iniwrite, %ClearingMode%, %SettingName%, Battle, ClearingMode
...
Critical
if (GdipImageSearch(x, y, "img/Emulator_Desktop.png", 100, 8, 1114, 36, 1214, 74)) ;遊戲閃退 位於模擬器桌面
{
								...
								if (Check71=Check72 and Check71=Check73)
								{
									...
									if (Checkzz>=9)
									{
										...
										Loop, 10
										{
											...
											else if (GdipImageSearch(x, y, "img/DormPage_LeaveDorm.png", 100, 8, 6, 63, 70, 117))
											{
												C_Click(x, y)
												sleep 3000
											}
											else if (GdipImageSearch(x, y, "img/Button_Formation.png", 100, 8, 744, 403, 981, 467))
												break
											sleep 300
										}
										sleep 5000
										if (GdipImageSearch(x, y, "img/Button_Formation.png", 100, 8, 744, 403, 981, 467))
										{
											LogShow("成功返回首頁")
										}
										else
...
if (True)
{
	...
	Formation := GdipImageSearch(x, y, "img/Button_Formation.png", 100, 8, 744, 403, 981, 467) ? 1 : 0 ;編隊BTN
	if (Formation)
	{
		WeighAnchor := GdipImageSearch(x, y, "img/Button_Main_Battle.png", 100, 8, 1017, 389, 1138, 451) ;出擊BTN
		MissionCheck := GdipImageSearch(x, y, "img/MainPage_MissionDone.png", 100, 8, 940, 704, 958, 718) ;任務完成
		MissionCheck2 := GdipImageSearch(x, y, "img/MainPage_N_Done.png", 100, 8, 38, 174, 63, 192) ;左邊任務的N
		...
		Living_AreaCheck := GdipImageSearch(x, y, "img/MainPage_HQ_Done.png", 100, 8, 623, 705, 641, 719) ;生活圈驚嘆號
		if (AcademySub and Living_AreaCheck and Formation and WeighAnchor and AcademyDone<1) ;學院
		{
			...
			Loop
			{
				if (GdipImageSearch(x, y, "img/Icon_Waiting_Academy.png", 100, 8, 173, 484, 371, 522)) ;等待進入學院/後宅選單
					break
				else if (GdipImageSearch(x, y, "img/Button_Formation.png", 100, 8, 744, 403, 981, 467)) ;沒有正確進入選單
					AC_Click(501, 713, 624, 738)
				else if (A_index>=100)
				{
			...
			sleep 1200
			if (GdipImageSearch(x, y, "img/Icon_Academy_OK.png", 100, 8, 358, 258, 380, 276)) ;
			{
				LogShow("執行學院任務！")
				gosub, AcademySub
			}
		...
		if (DormSub and Living_AreaCheck and Formation and WeighAnchor and DormDone<1)  ;後宅
		{
			...
			sleep 1200
			if (GdipImageSearch(x, y, "img/Icon_Dorm_OK.png", 100, 8, 747, 257, 771, 277)) ;
			{
				LogShow("執行後宅任務！")
				gosub, DormSub
			}
...
AnchorSub: ;出擊
if (GdipImageSearch(x, y, "img/Button_Main_Battle.png", 100, 8, 1017, 389, 1138, 451) and ((StopAnchor<1 and AnchorMode!="停用") or GotoOperation))
{
	AC_Click(1025, 356, 1145, 453) ;於首頁點擊點擊右邊"出擊"
	sleep 2000
}
else if (GdipImageSearch(x, y, "img/Icon_Surface.png", 100, 8, 16, 602, 151, 635)) ;在出擊的編隊頁面
{
	...
	if (Retreat_P1_LowHp)
	{
		Guicontrol,, starttext, % "P1: " P1_NowHp " % P2: " P2_NowHp " % P3: " P3_NowHp " %"
    ...
    if (GdipImageSearch(x, y, "img/AutoBattle_off.png", 100, 8, 743, 142, 806, 172) and Autobattle="自動") ;Auto Battle >> ON
    {
		LogShow("開啟自律模式")
        C_Click(819, 160)
    }
	else if (GdipImageSearch(x, y, "img/AutoBattle_On.png", 100, 8, 749, 143, 800, 173) and Autobattle="半自動")
	{
		LogShow("開啟半自動模式")
		C_Click(819, 160)
	}
	else if (GdipImageSearch(x, y, "img/AutoBattle_On.png", 100, 8, 749, 143, 800, 173) and Autobattle="關閉")
	{
		LogShow("關閉自律模式")
		C_Click(819, 160)
	}
	if (GdipImageSearch(x, y, "img/Icon_Kit.png", 40, 8, 91, 541, 135, 599) and (Use_FixKit)) ;如果有維修工具
	{
		Loop
		{
			if (GdipImageSearch(x, y, "img/Icon_Kit.png", 40, 8, 91, 541, 135, 599)) ;如果有維修工具
			{
				C_Click(119, 570) ;使用維修工具
				sleep 500
			}
			if (GdipImageSearch(x, y, "img/Icon_Kit_2.png", 100, 8, 397, 332, 455, 387)) ; 跳出訊息選單
			{
				C_Click(755, 483) ;點擊使用
				sleep 1500
			}
			if (GdipImageSearch(x, y, "img/Icon_Kit_2.png", 100, 8, 397, 332, 455, 387)) ;如果HP是滿的
			{
				C_Click(528, 483) ;點擊取消
			}
			sleep 300
			if (GdipImageSearch(x, y, "img/Icon_Surface.png", 100, 8, 16, 602, 151, 635)) ;回到編隊頁面
			{
				break
			}
			else if (A_index>100)
			{
				LogShow("使用維修工具過程中發生錯誤")
				break
			}
		}
	}
    ...
    if (GdipImageSearch(x, y, "img/Button_Confirm_Center.png", 100, 8, 700, 533, 865, 578)) ;心情低落
    {
		if mood=強制出戰
		{
    ...
    if (GdipImageSearch(x, y, "img/Text_OutofOil.png", 100, 8, 465, 344, 481, 373)) ;石油不足
    {
...
else if (GdipImageSearch(x, y, "img/Button_Retreat.png", 100, 8, 763, 697, 919, 731))
{	
	...
	Loop, 100
	{
		...
		if (GdipImageSearch(x, y, "img/Icon_Info.png", 100, 8, 1181, 403, 1274, 428))
		{
			LogShow("關閉陣型列表")
			C_Click(1071, 476)
		}
		if (bulletFailed<1 and Item_Bullet) ;只有在彈藥歸零時才會拾取
		{
			...
			and GdipImageSearch(n, n, "img/SubChapter/Bullet_None.png", 10, SearchDirection, MapX1, MapY1, MapX2, MapY2))
			{
				...
				Loop, 3
				{
					...
					if (GdipImageSearch(x, y, "img/Button_Retreat.png", 100, 8, 763, 697, 919, 731)) ;如果在限時(無限時)地圖
					{
						C_Click(xx, yy)
						if (GdipImageSearch(x, y, "img/Unable_Way.png", 100, 8, 422, 348, 439, 374))
						{
							bulletFailed++
							break
						}
					...
					if (GdipImageSearch(x, y, "img/Icon_Surface.png", 100, 8, 16, 602, 151, 635)) ;規避失敗
					{
						Break
					}
		...
		if (Bossaction="優先攻擊－當前隊伍" or Bossaction="優先攻擊－切換隊伍" or Bossaction="撤退") ;ＢＯＳＳ
		{
			if (GdipImageSearch(x, y, BOSSICO, 15, SearchDirection, MapX1, MapY1, MapX2, MapY2) and BossFailed<1)
			{
				...
				else if (movemap(x, y)) ;如果在左上角可能誤點
				{
					return
				}
				else if (Bossaction="優先攻擊－當前隊伍")
				{
					...
					Loop, 15
					{
						xx := x+20
						yy := y
						if (GdipImageSearch(x, y, "img/Button_Retreat.png", 100, 8, 763, 697, 919, 731) and xx>147 and yy>200 and xx<MapX2 and yy<MapY2) 
						{
							C_Click(xx, yy)
							sleep 400
							FindWay(xx, yy)
						...
						else if (GdipImageSearch(x, y, "img/Button_Retreat.png", 100, 8, 763, 697, 919, 731) and xx<290 and yy<195) 
						{
						...
						if (GdipImageSearch(x, y, "img/Icon_Surface.png", 100, 8, 16, 602, 151, 635)) ;進入戰鬥界面
						{
							Break
						}
						Ambush()
				...
				else if (Bossaction="優先攻擊－切換隊伍")
				{
					...
					if (SwitchParty<1)
					{
						...
						sleep 300
						if (GdipImageSearch(x, y, "img/Text_NoFleettoChangeWith.png", 100, 8, 467, 343, 482, 372)) ;沒有艦隊可以切換
						{
						}
						else 
						{
					...
					else
					{
						...
						sleep 4050
						Ambush()
						if !(GdipImageSearch(x, y, "img/Icon_Surface.png", 100, 8, 16, 602, 151, 635) and BossFailed<1) ; 如果沒有成功進入戰鬥，再試一次
						{
							C_Click(xx, yy)
							sleep 2050
						}
		...
		else if (FindFleet_2(x, y, FP, SearchDirection, MapX1, MapY1, MapX2, MapY2)) ;偵查OO艦隊
		{
			...
			Loop, 10
			{
				...
				if (GdipImageSearch(x, y, "img/Icon_Surface.png", 100, 8, 16, 602, 151, 635))
				{
		...
		if (SearchLoopcount>=2 and GdipImageSearch(x, y, "img/Button_Retreat.png", 100, 8, 763, 697, 919, 731))
		{
		...
		SearchLoopcount++
	} until !GdipImageSearch(x, y, "img/Button_Retreat.png", 100, 8, 763, 697, 919, 731)
}
else if (GdipImageSearch(x, y, "img/Icon_Attack.png", 100, 8, 117, 48, 212, 81)) ;在出擊選擇關卡的頁面
{
	...
	if (OperationSub and !OperationDone)
	{
		...
		else
		{
			if (Operation_ReLogin and !IsOperation_ReLogin) 
			{
				...
				LogShow("演習前重登。")
				runwait, %Consolefile% killapp --index %emulatoradb% --packagename "com.YoStarEN.AzurLane" , %ldplayer%, Hide
				Loop
				{
					if (GdipImageSearch(x, y, "img/Emulator_Desktop.png", 100, 8, 1114, 36, 1214, 74))
						break
					else if (A_index>=60) 
					{
				...
				runwait, %Consolefile% runapp --index %emulatoradb% --packagename "com.YoStarEN.AzurLane" , %ldplayer%, Hide
			...
			if (GdipImageSearch(x, y, "img/Button_Exercise.png", 100, 8, 1108, 676, 1142, 721))
			{ ;如果在出擊頁面檢查到演習還沒執行
				if (Operation_ReLogin and IsOperation_ReLogin)
				{
				...
				Loop
				{
					if (GdipImageSearch(x, y, "img/Button_Exercise.png", 100, 8, 1108, 676, 1142, 721)) ;如果在出擊頁面檢查到演習還沒執行
					{
						C_Click(1177, 706) ;嘗試進入演習頁面
						sleep 3000
					}
					if (GdipImageSearch(x, y, "img/Icon_Exercise.png", 100, 8, 117, 44, 256, 85)) ;左上"演習"
					{
						Break ;成功進入每日頁面
					}
				}
	...
	Chapter1 := GdipImageSearch(x, y, "img/Stage_1_1.png", 100, 8, 185, 515, 220, 542) ? 1 : 0  ;第一關 1-1
	Chapter2 := GdipImageSearch(x, y, "img/Stage_2_1.png", 100, 8, 852, 517, 884, 541) ? 1 : 0  ;第二關 2-1
	Chapter3 := GdipImageSearch(x, y, "img/Stage_3_1.png", 100, 8, 439, 281, 474, 306) ? 1 : 0  ;第三關 3-1
	...
	if (AnchorChapter>=1 and AnchorChapter<=14)
	{
		...
		{
			if (GdipImageSearch(x, y, "img/Stage_1_1.png", 100, 8, 186, 516, 218, 542))
			{
				C_Click(221,526)
			}
		}
		...
		{
			if (GdipImageSearch(x, y, "img/Stage_1_2.png", 100, 8, 492, 334, 526, 358))
			{
				C_Click(510,342)
			}
		}
		...
		{
			if (GdipImageSearch(x, y, "img/Stage_1_3.png", 100, 8, 697, 591, 730, 615))
			{
				C_Click(713,600)
			}
		}
		...
		{
			if (GdipImageSearch(x, y, "img/Stage_1_4.png", 100, 8, 824, 240, 857, 263))
			{
				C_Click(862,247)
			}
		}
		...
		{
			if (GdipImageSearch(x, y, "img/Stage_2_1.png", 100, 8, 852, 517, 884, 541))
			{
				C_Click(x, y)
			}
		}
		...
		{
			if (GdipImageSearch(x, y, "img/Stage_2_2.png", 100, 8, 785, 248, 823, 273))
			{
				C_Click(x, y)
			}
		}
		...
		{
			if (GdipImageSearch(x, y, "img/Stage_2_3.png", 100, 8, 325, 332, 360, 355))
			{
				C_Click(x, y)
			}
		}
		...
		{
			if (GdipImageSearch(x, y, "img/Stage_2_4.png", 100, 8, 420, 608, 457, 631))
			{
				C_Click(438,620)
			}
		}
		...
		{
			if (GdipImageSearch(x, y, "img/Stage_3_1.png", 100, 8, 439, 281, 474, 306))
			{
				C_Click(x, y)
			}
		}
		...
		{
			if (GdipImageSearch(x, y, "img/Stage_3_2.png", 100, 8, 268, 562, 303, 585))
			{
				C_Click(x, y)
			}
		}
		...
		{
			if (GdipImageSearch(x, y, "img/Stage_3_3.png", 100, 8, 825, 197, 862, 220))
			{
				C_Click(867,209)
			}
		}
		...
		{
			if (GdipImageSearch(x, y, "img/Stage_3_4.png", 100, 8, 651, 426, 688, 448))
			{
				C_Click(691,433)
			}
		}
...
Loop
{
	...
	if (Operation_Only and Operation_OnlyNum=OperationFightCount and GdipImageSearch(x, y, "img/Icon_Exercise.png", 100, 8, 117, 44, 256, 85))
	{
	...
	else if (GdipImageSearch(x, y, "img/Icon_Exercise.png", 100, 8, 117, 44, 256, 85))  ;演習介面隨機
	{
		if (Operationenemy="最弱的")
		{
			Capture2(234, 298, 293, 320, 1)   ;第一位敵人主力
			enemy1 := OCR("capture/OCRTemp.png")
			Capture2(481, 298, 538, 320, 1)   ;第二位敵人主力
			enemy2 := OCR("capture/OCRTemp.png")			
			Capture2(720, 298, 783, 320, 1)  ;第三位敵人主力
			enemy3 := OCR("capture/OCRTemp.png")
			Capture2(963, 298, 1027, 320, 1) ;第四位敵人主力
	...
	else if (GdipImageSearch(x, y, "img/Exercise_Start.png", 100, 8, 590, 585, 691, 624)) ;演習對手訊息
	{
		C_Click(x, y)
		sleep 500
	}
	else if (GdipImageSearch(x, y, "img/Button_Exercise_Battle.png", 100, 8, 1054, 647, 1172, 686)) ;編隊畫面
	{
		LogShow("演習出擊。")
		C_Click(1089, 689)
		sleep 1300
		if (GdipImageSearch(x, y, "img/Button_Exercise_Battle.png", 100, 8, 1054, 647, 1172, 686))
		{
			LogShow("演習結束！")
			Iniwrite, %OperationToday%, %SettingName%, Battle, OperationYesterday
			C_Click(1239, 72) ;回到首頁
		...
		sleep 5000
		if (GdipImageSearch(x, y, "img/Button_Exercise_Battle.png", 100, 8, 1054, 647, 1172, 686)) ;點了出擊過了5秒還是沒出擊
		{
			LogShow("演習異常，強制結束！")
			Iniwrite, %OperationToday%, %SettingName%, Battle, OperationYesterday
			C_Click(1239, 72) ;回到首頁
	...
	else if (GdipImageSearch(x, y, "img/Exercise_Supply.png", 100, 8, 111, 43, 241, 80)) ;誤點商店
	{
...
sleep 500
runwait, %Consolefile% launchex --index %emulatoradb% --packagename "com.YoStarEN.AzurLane" , %ldplayer%, Hide
sleep 10000
Winget, UniqueID,, %title%
Global UniqueID
Loop
{
	...
	if (GdipImageSearch(x, y, "img/Icon_CRIWARE.png", 100, 8, 1219, 694, 1267, 727))
	{
		LogShow("位於遊戲首頁，自動登入")
		sleep 5000
		C_Click(642, 420)
	...
	else if (GdipImageSearch(x, y, "img/Button_Main_Battle.png", 100, 8, 1017, 389, 1138, 451))
	{
		LogShow("自動登入完畢。")
		break
	}
...
if  (DailyGoalSub and DailyDone<1)
{
	...
	if (Yesterday=Today)
	{
		...
		Loop
		{
			...
			if (GdipImageSearch(x, y, "img/Button_Main_Battle.png", 100, 8, 1017, 389, 1138, 451)) ;如果成功返回首頁
			{
				Break
			}
		}
	...
	else
	{
		...
		Loop
		{
				...
				if (Find(x, y, 427, 328, 527, 388, Daily_Ico)) ;如果出現驚嘆號 (多確認一個紅尖尖 避免誤判)
				{
					...
					else
					{
						...
						Loop, 30
						{
							...
							if (GdipImageSearch(x, y, "img/Button_Main_Battle.png", 100, 8, 1017, 389, 1138, 451))
							{
								DailyBreak := 1
								DailyDone := 1
								Logshow("每日任務已結束")
			...
			else if (Find(x, y, 100, 34, 200, 94, Daily_Formation))
			{
				...
				C_Click(1147, 667)
				if (GdipImageSearch(x, y, "img/Button_Confirm_Center.png", 100, 8, 700, 533, 865, 578))
				{
				...
				else if (GdipImageSearch(x, y, "img/Text_OutofOil.png", 100, 8, 465, 344, 481, 373))
				{
					Logshow("石油不足，停止每日任務。")
					StopAnchor := 1
					Loop, 20
...
if (MissionCheck) ;如果有任務獎勵
{
	...
	Loop
	{
		if (GdipImageSearch(x, y, "img/MainPage_MissionDone.png", 100, 8, 940, 704, 958, 718)) 
		{
			C_Click(883, 725) ;點擊任務按紐
			sleep 1000
		}
		...
		sleep 500
	} until GdipImageSearch(x, y, "img/MissionPage_All.png", 100, 8, 32, 156, 76, 225) ;等待進入任務界面 (偵測金色的"全部")
    Loop
    {
        if (GdipImageSearch(x, y, "img/MissionPage_CollectAll.png", 100, 8, 1035, 47, 1100, 81)) ;全部領取任務獎勵
        {
            LogShow("領取全部任務獎勵！")
            C_Click(1068, 63)
			sleep 3000
        }
        else if (GdipImageSearch(x, y, "img/MissionPage_Collect.png", 100, 8, 1084, 167, 1189, 200)) ;領取第一個任務獎勵
        {
            C_Click(1136, 187)
        }
		else if (GdipImageSearch(x, y, "img/Button_Story_Skip.png", 100, 8, 1179, 63, 1282, 87)) ;劇情
        {
            C_Click(x, y)
        }
		else if (GdipImageSearch(x, y, "img/Item_Found.png", 100, 8, 542, 100, 745, 400)) ;獲得道具
		{
			C_Click(x, y)
		}
        else if (GdipImageSearch(x, y, "img/MissionPage_Go.png", 100, 8, 1104, 164, 1171, 202))
        {
		...
		else if (GdipImageSearch(x, y, "img/Button_Formation.png", 100, 8, 744, 403, 981, 467) and MissionDone=1)
		{
			MissionDone :=0
			break
		}
...
if (MissionCheck2) ;在主選單偵測到軍事任務已完成
{
	...
	Loop
	{
		if (GdipImageSearch(x, y, "img/MainPage_N_Done.png", 100, 8, 38, 174, 63, 192))
		{
			C_Click(20, 200)
			sleep 1000
		}
		if (GdipImageSearch(x, y, "img/Commission_Completed.png", 100, 8, 383, 304, 504, 329)) ;出現選單"完成"
		{
			C_Click(x, y) ;點擊軍事委託完成
			sleep 2500
		}
		if (GdipImageSearch(x, y, "img/Commission_WellDone.png", 100, 8, 670, 65, 794, 107)) ;出現委託成功S頁面
		{
			break
		}
		else if (A_Index>200)
	...
	{
		sleep 400
		if (GdipImageSearch(x, y, "img/Commission_WellDone.png", 100, 8, 670, 65, 794, 107)) ;委託成功 S
		{
			C_Click(x, y) ;隨便點
			sleep 1000
		}
		else if (GdipImageSearch(x, y, "img/Commission_Available.png", 100, 8, 224, 339, 285, 365)) ;如果已經"空閒"
		{
			sleep 1000
			if (GdipImageSearch(x, y, "img/Commission_Available.png", 100, 8, 224, 339, 285, 365))
			{
				C_Click(441, 314)
				sleep 1500
			}
		}
		else if (GdipImageSearch(x, y, "img/Commission_Daily.png", 100, 8, 31, 161, 75, 206)) ;成功進入委託頁面
		{
			Rmenu := 1
			break
		}
	...
	if (Rmenu=1)
	{
		...
		Loop
		{
			if (GdipImageSearch(x, y, "img/Commission_PurpleText.png", 100, 8, 13, 285, 204, 316))
			{
				C_Click(1246, 89)
				sleep 1000
			}
...
if (AcademyDone<1)
{
	...
	Loop ;等待進入學院
	{
		...
		if (GdipImageSearch(x, y, "img/AcademyPage_Academy.png", 100, 8, 119, 53, 238, 75))
		{
			break
		}
		else if (A_index=200)
	...
	Loop
	{
		...
		if (GdipImageSearch(x, y, "img/AcademyPage_Shop.png", 100, 8, 1038, 208, 1099, 234) and AcademyShop and AcademyShopDone<1)
		{
			LogShow("商店街發大財")
			C_Click(x, y)
			Loop, 20
			{
				sleep 500
			} until GdipImageSearch(x, y, "img/AcademyPage_into_Shop.png", 100, 8, 114, 53, 238, 77) ? 1 : 0 ;檢查是否進入商店
			...
			Loop
			{
				...
				if (Find(x, y, ShopX1, ShopY1, ShopX2, ShopY2, Item_OuterEqBox) and Item_Equ_Box1 and Item_Equ_Box1Coin<1) ;如果有外觀裝備箱
				{
					...
					Loop, 20
					{
						...
						if (GdipImageSearch(x, y, "img/Shop_exchange.png", 100, 8, 707, 541, 874, 575)) ;跳出購買訊息
						{
							Random, xx, 713, 863
							Random, yy, 543, 569
				...
				{
					if (GdipImageSearch(x, y, "img/Shop_Merit.png", 100, 8, 944, 665, 985, 733)) ;軍需商店
					{
						C_Click(x, y)
					}
		...
		if (GdipImageSearch(x, y, "img/AcademyPage_Tactical.png", 100, 8, 713, 191, 763, 213) and AcademyTactics and learnt<1) ;學院出現！
		{
			LogShow("我們真的學不來！")
		...
		if (Classroom)
		{
			...
			if (Find(x, y, 480, 204, 580, 259, ClassRoomDone) and Checkweek!=1) ;大講堂出現！ 星期天不執行
			{
				...
				Try ;開始篩選 ; 
				{
					...
					} until GdipImageSearch(x, y, "img/Icon_Dock_Sort.png", 100, 8, 64, 135, 145, 171)
					sleep 1000
		...
		if (A_index>10)
		{
			...
			Loop, 60
			{
				if (GdipImageSearch(x, y, "img/AcademyPage_Academy.png", 100, 8, 119, 53, 238, 75))
					C_Click(38, 92)
				else if !(GdipImageSearch(x, y, "img/AcademyPage_Academy.png", 100, 8, 119, 53, 238, 75))
				{
					sleep 1000
					if !GdipImageSearch(x, y, "img/AcademyPage_Academy.png", 100, 8, 119, 53, 238, 75)
						break
...
{
	Guicontrol,, starttext, 目前狀態：視窗被放大，等待恢復
}
else
{
	WinGetPos, wx, wy, ww, wh, %title%
	if (LDplayer and (ww!=1318 or wh!=758))
	{
		LogShow("視窗大小不為1280x720，自動調整。")
		WinMove,  %title%, , , , %EmulatorResolution_W%, %EmulatorResolution_H%
	}
}
return
...
if (DormDone<1) ;後宅發現任務
{
	...
	Loop ;等待進入後宅
	{
		...
		if (GdipImageSearch(x, y, "img/DormPage_LeaveDorm.png", 100, 8, 6, 63, 70, 117))
		{
			sleep 500
			break
		}
		else if (GdipImageSearch(x, y, "img/DormPage_Text_commander.png", 100, 8, 1040, 649, 1175, 676))
		{
	...
	{
		GuLuGuLuLu() ;如果太過飢餓 
		if (GdipImageSearch(x, y, "img/DormPage_Train.png", 100, 8, 83, 116, 217, 174))
			C_Click(1261, 464) ;點到訓練自動離開
		if (GdipImageSearch(x, y, "img/DormPage_Expansion_Cancel.png", 100, 8, 408, 495, 561, 533))
			C_Click(x, y) ;點到施工自動離開
		if (Find(x, y, 592, 477, 692, 537, DormPage_Cancel))
			C_Click(x, y) ;點到換層自動離開
		if (GdipImageSearch(x, y, "img/DormPage_Supplies.png", 100, 8, 273, 275, 416, 321))
			C_Click(617, 115) ;點到存糧自動離開
		if (GdipImageSearch(x, y, "img/DormPage_Subject.png", 100, 8, 1077, 522, 1204, 556))
			C_Click(37, 90) ;點到管理自動離開
		if (GdipImageSearch(x, y, "img/DormPage_Subject_Shop.png", 100, 8, 67, 70, 274, 114))
			C_Click(1200, 68) ;點到商店自動離開
		if (GdipImageSearch(x, y, "img/DormPage_Inform_Confirm.png", 100, 8, 561, 548, 719, 592))
			C_Click(638, 545) ;點到訊息自動離開
		;~ if (Find(x, y, 301, 87, 401, 147, DormPage_Share))
			;~ C_Click(1299, 646) ;點到分享自動離開
		if (GdipImageSearch(x, y, "img/DormPage_Text_commander.png", 100, 8, 1040, 649, 1175, 676))
			C_Click(1110, 657) ;獲得經驗 按確定
		if (GdipImageSearch(x, y, "img/DormPage_NickName_Cancel.png", 100, 8, 447, 466, 583, 503))
		...
		if (DormFood and DormFoodDone<1)
		{
			...
			FoodCheck2 := GdipImageSearch(x, y, "img/DormPage_YellowCross.png", 100, 8, 34, 673, 65, 703) ? 1 : 0 ;左下黃十字
...
DelegationMission() {
	Loop
	{
		if (GdipImageSearch(x, y, "img/Commission_Daily.png", 100, 8, 31, 161, 75, 206)) ;左邊金色每日
			break
		sleep 500
	}
	Loop
	{
		...
		else if (GdipImageSearch(x, y, "img/Commission_InAction.png", 100, 8, 368, 193, 447, 217) 
		or GdipImageSearch(x, y, "img/Commission_None.png", 100, 8, 368, 193, 449, 217)) ;任務都在進行中 or 都沒接到任務
		{
			break
		}
	...
	{
		sleep 500
	} until GdipImageSearch(x, y, "img/Commission_Urgent.png", 100, 8, 32, 262, 74, 301) ;等待切換到緊急頁面
	Loop
	{
		sleep 200
		if (Find(x, y, 133, 107, 233, 162, Delegation_Page_Done)) 
		...
		else if (GdipImageSearch(x, y, "img/Icon_Commission.png", 100, 8, 120, 50, 305, 79))
		{
			failedcount++
			if (failedcount>20)
				break
	...
	if (GdipImageSearch(x, y, "img/Commission_None.png", 100, 8, 177, 193, 449, 725))  ;接獲緊急任務
	{
		DelegationMission3()
	}
	C_Click(53, 191) ;每日
	...
	Loop, 30
	{
		if (GdipImageSearch(x, y, "img/Icon_Commission.png", 100, 8, 120, 50, 305, 79))
		{
			C_Click(53, 89) ;離開
			sleep 2000
		}
...
{
	if (GdipImageSearch(x, y, "img/Commission_None.png", 100, 8, 177, 193, 449, 222)) ;第一個任務未開始
	{
		LogShow("Mission1 := 0")
		Mission1 := 0
	}
	if (GdipImageSearch(x, y, "img/Commission_None.png", 100, 8, 177, 332, 449, 369)) ;第二個任務未開始
	{
		LogShow("Mission2 := 0")
		Mission2 := 0
	}
	if (GdipImageSearch(x, y, "img/Commission_None.png", 100, 8, 177, 480, 449, 514)) ;第三個任務未開始
	{
		LogShow("Mission3 := 0")
		Mission3 := 0
	}
	if (GdipImageSearch(x, y, "img/Commission_None.png", 100, 8, 177, 624, 449, 660)) ;第四個任務未開始
	{
		LogShow("Mission4 := 0")
		Mission4 := 0
	}
	if (Mission1 = 0 and !GdipImageSearch(x, y, "img/Commission_Done.png", 100, 8, 1073, 46, 1096, 80))
	{
		C_Click(560, 192)
		Mission1 := DelegationMission2()
		if (Mission1=2 and !GdipImageSearch(x, y, "img/Commission_Done.png", 100, 8, 1073, 46, 1096, 80) and Mission4=0)
		{
	...
	if (Mission2 = 0 and !GdipImageSearch(x, y, "img/Commission_Done.png", 100, 8, 1073, 46, 1096, 80))
	{
		C_Click(560, 332)
	...
	if (Mission3 = 0 and !GdipImageSearch(x, y, "img/Commission_Done.png", 100, 8, 1073, 46, 1096, 80))
	{
		C_Click(560, 471)
...
DelegationMission2()
{
	...
	{
		sleep 300
		if (GdipImageSearch(x, y, "img/Button_Recommend.png", 100, 8, 870, 370, 1002, 403))
		{
			loopcount := 0
			break
		}
	...
	;~ LogShow("成功進入")
	if (DwmCheckcolor(1138, 338, 4870499) or GdipImageSearch(x, y, "img/Icon_Oil.png", 100, 8, 720, 160, 950, 200) or Find(x, y, 1060, 151, 1160, 211, Ico_Gem)) ;如果耗油是個位數 或 出現寶石 或 出現油田
	{
		C_Click(931, 380)
		sleep 300
		if (GdipImageSearch(x, y, "img/Commission_Ready.png", 100, 8, 1043, 368, 1133, 407) 
		and GdipImageSearch(x, y, "img/Commission_10_oil_only.png", 100, 8, 1079, 325, 1128, 350)) ;如果成功推薦角色
		{
			C_Click(1096, 385) ;開始
			sleep 1000
...
{
	if (GdipImageSearch(x, y, "img/Battle_Victory_V.png", 70, 8, 807, 397, 849, 433))
	{	
		if (GdipImageSearch(x, y, "img/Battle_Defeat.png", 70, 8, 811, 449, 850, 485)) ; 有隊員倒下
		{
			message = 敵艦討伐完畢（隊員受重創）。
		}
		else if (GdipImageSearch(x, y, "img/Battle_Defeat.png", 70, 8, 811, 501, 850, 540)) ; XX
		{
	...
	else if (GdipImageSearch(x, y, "img/Battle_Defeat_Upgrade_Close.png", 100, 8, 592, 636, 671, 670)) ;指揮官可以通過以下方式提升艦隊實力
	{
		LogShow("提督SAMA請提升實力。")
		AC_Click(537, 639, 715, 666)
	}
	else if (GdipImageSearch(x, y, "img/Battle_Defeat.png", 70, 8, 811, 396, 850, 434)) ;點擊繼續
	{
...
GetItem()
{
	if (GdipImageSearch(x, y, "img/Item_Found.png", 100, 8, 530, 130, 780, 360)) ;獲得道具
	{
		LogShow("獲得道具，點擊繼續！") ; Found Item, Press to countine!
		C_Click(638, 519)
		return 1
	}
}
...
GetCard()
{
	if (GdipImageSearch(x, y, "img/Icon_GetCard.png", 100, 8, 1076, 631, 1118, 674)) ;獲得新卡片
	{
		sleep 1500
		Capture() ;拍照
...
Message_Center()
{
	if (GdipImageSearch(x, y, "img/Icon_TipsGril.png", 100, 8, 633, 328, 699, 382))
	{
		LogShow("每日提示，今日不再顯示！")
		C_Click(827, 518)
		C_Click(641, 578)
	}
	else if (GdipImageSearch(x, y, "img/Button_Confirm_Center.png", 100, 8, 569, 536, 712, 579)) ;中央訊息 按鈕在下方
	{
		LogShow("中央訊息，點擊確認！")
		C_Click(635, 542)
		return 1
...
Battle_End()
{
	if (GdipImageSearch(x, y, "img/Button_Confirm_Orange.png", 100, 8, 1022, 676, 1155, 717)) ;確定
	{
		LogShow("結算畫面，點擊確定！")
		C_Click(x, y)
		sleep 3000
...
Message_Story()
{
	 if (GdipImageSearch(x, y, "img/Button_Story_Skip.png", 100, 8, 1179, 63, 1282, 87))
	{
		LogShow("劇情對話，自動略過")
		C_Click(1200, 74)
		sleep 1500
...
Ambush()
{
	if (GdipImageSearch(x, y, "img/Button_Evade.png", 80, 8, 1013, 487, 1121, 527)) ;遇襲
	{
		if Assault=迎擊
		{
			LogShow("遇襲：迎擊！")
...
{
	global StopAnchor
	if (GdipImageSearch(x, y, "img/Button_Sort_Dock.png", 100, 8, 388, 539, 494, 578))
	{
		if shipsfull=停止出擊
		{
			LogShow("船䲧已滿：停止出擊。")
			...
			Loop
			{
				if (GdipImageSearch(x, y, "img/Icon_Surface.png", 100, 8, 16, 602, 151, 635)) ;進入戰鬥的編隊頁面 
				{
					C_Click(1227,71)
					sleep 1000
				}
				...
				if (GdipImageSearch(x, y, "img/Button_Main_Battle.png", 100, 8, 1017, 389, 1138, 451)) ;回到首頁
				{
					StopAnchor := 1 ;不再出擊
					return StopAnchor
					Break
		...
		else if shipsfull=整理船䲧
		{
			...
			} until GdipImageSearch(x, y, "img/Icon_Dock.png", 100, 8, 114, 47, 195, 81)
			Gosub, Anchorsettings ;Get GUI settings again
			sleep 500
			Loop
			{
				if (GdipImageSearch(x, y, "img/Icon_Dock.png", 100, 8, 114, 47, 195, 81))
				{
					C_Click(1136, 64) ;開啟篩選
					Loop
					{
					...
					if (GdipImageSearch(x, y, "img/Icon_Dock_Sort.png", 100, 8, 64, 135, 145, 171))
					{
						if ((Rarity1) and GdipImageSearch(x, y, "img/Dock_Rarity_Common_off.png", 50, 8, 443, 478, 581, 514)) 
						or ((Rarity2) and GdipImageSearch(x, y, "img/Dock_Rarity_Rare_off.png", 50, 8, 602, 479, 734, 513)) 
						or ((Rarity3) and GdipImageSearch(x, y, "img/Dock_Rarity_Elite_off.png", 50, 8, 760, 479, 891, 515))
						{
						...
						if ((Rarity1) and !GdipImageSearch(x, y, "img/Dock_Rarity_Common_on.png", 50, 8, 444, 477, 577, 517)) 
						or ((Rarity2) and !GdipImageSearch(x, y, "img/Dock_Rarity_Rare_on.png", 50, 8, 601, 477, 737, 517)) 
						or ((Rarity3) and !GdipImageSearch(x, y, "img/Dock_Rarity_Elite_on.png", 50, 8, 758, 477, 894, 516))
						{
						...
						sleep 1000
						if (GdipImageSearch(x, y, "img/Text_Dock_Found_no_Ships.png", 100, 8, 94, 377, 525, 420)) ;如果篩選完畢發現沒有船可以退役
						{
			...
			Loop
			{
				if (GdipImageSearch(x, y, "img/Icon_Dock_Info.png", 100, 8, 182, 80, 272, 124))
					C_Click(1014,677)  ;退役確定 
				else if (GdipImageSearch(x, y, "img/Item_Found.png", 100, 8, 542, 168, 745, 284)) ;獲得道具(一行)
					C_Click(x, y)
				else if (GdipImageSearch(x, y, "img/Button_Dock_Disa_Equ_Confirm.png", 100, 8, 892, 561, 1024, 597)) ;拆裝(確定)
					C_Click(x, y)
				else if (GdipImageSearch(x, y, "img/Button_Dock_WillGet_Disass.png", 100, 8, 722, 586, 894, 622)) ;將獲得以下材料
					C_Click(x, y)
				else if (GdipImageSearch(x, y, "img/Text_Dock_Found_no_Ships.png", 100, 8, 94, 377, 525, 420)) ;暫無符合條件的艦船
				{
					sleep 300
					C_Click(64, 91)
					Logshow("退役結束")
				...
				else if (GdipImageSearch(x, y, "img/Icon_Dock.png", 100, 8, 114, 47, 195, 81) 
				and !GdipImageSearch(x, y, "img/Text_Dock_Found_no_Ships.png", 100, 8, 94, 377, 525, 420)) ;第一位還沒被退役
				{
					DockCount++
					if (DockCount>30 and GdipImageSearch(x, y, "img/Icon_Dock.png", 100, 8, 114, 47, 195, 81)) ;偵測"船塢"
					{
				...
				{
					IsDockCount++
					if (IsDockCount>300 and GdipImageSearch(x, y, "img/Icon_Dock.png", 100, 8, 114, 47, 195, 81)) ;偵測"船塢"
					{
						C_Click(64, 91) ;避免出現一些問題(例如船未上鎖)，強制結束退役
						IsDockCount := 0
...
ToMap()
{
	if (GdipImageSearch(x, y, "img/Button_Go.png", 100, 8, 872, 536, 1040, 577))
	{
		if (ClearingMode) ;周回模式開關
		{
			if (GdipImageSearch(x, y, "img/ClearingMode_Off.png", 100, 8, 988, 619, 1065, 644)) ;
			{
				LogShow("開啟周回模式")
				C_Click(1022, 631)
			}
		}
		else if !(ClearingMode)
		{
			if (GdipImageSearch(x, y, "img/ClearingMode_On.png", 100, 8, 988, 619, 1066, 645))
			{
				LogShow("關閉周回模式")
				C_Click(1012, 631)
			}
...
{
	Global
	if (GdipImageSearch(x, y, "img/Select_Fleet.png", 100, 8, 122, 108, 330, 146))
	{
		LogShow("選擇出擊艦隊中。")
		if (AnchorMode="普通") and !(AnchorChapter="希望1")
		{
			C_Click(1142, 370) ;先清掉第二艦隊 Clear Fleet 2
			sleep 300
			C_Click(1060, 230) ;開啟第一艦隊的選擇選單 Open choose list of fleet 1
			sleep 500
			if (GdipImageSearch(x, y, "img/Button_SelectFleet_Choose.png", 100, 8, 1028, 343, 1091, 383)) 
				return ;如果選單沒有正確開啟 if list 1 not opened
			...
			{
				sleep 500
				C_Click(1053, 368)	;開啟第二艦隊的選擇選單 click fleet 2 choose list
				sleep 500
				if (GdipImageSearch(x, y, "img/Button_SelectFleet_Go.png", 100, 8, 1048, 617, 1109, 659)) 
				{ ;如果選單沒有正確開啟 if list 2 not opened
					C_Click(1161, 126)
					return
				}
			}
		...
		AC_Click(1000, 620, 1150, 655)	;立刻前往 Click "Go"
		sleep 500
		if (GdipImageSearch(x, y, "img/Button_Confirm_Center.png", 100, 8, 700, 533, 865, 578)) ;心情低落 Wife Angry.
		{
			LogShow("老婆心情低落中。")
			C_Click(743, 541)
		}
		else if (GdipImageSearch(x, y, "img/Text_OutofOil.png", 100, 8, 465, 344, 481, 373)) ; 資源不夠 have no oil to go.
		{
			LogShow("石油不足，停止出擊到永久。")
			StopAnchor := 1
			sleep 1000
			C_Click(1230, 68) ;返回主選單 return to main page
			sleep 2000
			return StopAnchor
		}
		else if GdipImageSearch(x, y, "img/Text_Not_Challenge_Anymore.png", 100, 8, 314, 344, 329, 375) and (AnchorMode="困難")
		{
			if (IsDailyHardMap)
			{
				LogShow("每日困難地圖執行結束。")
		...
		{
			sleep 500
			if (GdipImageSearch(x, y, "img/Button_Retreat.png", 100, 8, 763, 697, 919, 731)) ;如果進入地圖頁面 
			{
				if (SwitchPartyAtFirstTime and (ChooseParty2!="不使用" or AnchorMode="困難"))
				{
					Loop, 20 ;等待進入地圖的掃描結束 waiting for scan.
					{
					...
					sleep 3000
					AC_Click(970, 710, 1050, 720) ;點擊"切換" Click "Switch"
					sleep 1500
				}
...
Battle_Operation()
{
	...
	if (GdipImageSearch(x, y, "img/Button_Pause.png", 100, 8, 1148, 68, 1252, 101))
	{
			...
			if (Leave_Operatio and IsChanged<20) ;快輸了自動離開 重試20次
			{
				...
				if (Myhp<OperatioMyHpBar and Emhp>OperatioEnHpBar)
				{
					...
					if (Myhp<OperatioMyHpBar and Emhp>OperatioEnHpBar)   ;再檢查一次
					{
						...
						Loop, 100
						{
							if (GdipImageSearch(x, y, "img/Button_Pause.png", 100, 8, 1148, 68, 1252, 101)) ;點擊暫停按紐
							{
								Random, x, 1152, 1256
								Random, y, 69, 88
								C_Click(x, y)
							...
							else if (GdipImageSearch(x, y, "img/Button_LeaveBattle.png", 100, 8, 423, 536, 592, 569)) ;退出戰鬥
							{
								Random, x, 443, 567
								Random, y, 540, 569
								C_Click(x, y)
							...
							else if (GdipImageSearch(x, y, "img/Button_Exit.png", 100, 8, 752, 537, 827, 572)) ;確認退出
							{
								Random, x, 726, 862
								Random, y, 544, 570
								C_Click(x, y)
							...
							else if (GdipImageSearch(x, y, "img/Button_Exercise_Battle.png", 100, 8, 1054, 647, 1172, 686)) ;回到編隊出擊頁面
							{
								C_Click(59, 90) ;返回上一頁
							}
							else if (GdipImageSearch(x, y, "img/Icon_Exercise.png", 100, 8, 117, 44, 256, 85)) ;回到演習頁面
							{
					...
					if (IsPositive_Integer)
					{
						...
						if ((OriginalHP-NowHP)>=Retreat_LowHpBar and NotRetreat<1 and NowHP!=0) ;HP過低撤退
						{
							...
							Loop, 300
							{
								...
								else if (GdipImageSearch(x, y, "img/Button_Pause.png", 100, 8, 1148, 68, 1252, 101)) ;點擊暫停按紐
								{
									Random, x, 1152, 1256
									Random, y, 69, 88
									C_Click(x, y)
								...
								else if (GdipImageSearch(x, y, "img/Button_Attack.png", 100, 8, 1156, 696, 1255, 728)) ;迎擊
								{
									C_Click(x, y)
									sleep 1000
								}
								else if (GdipImageSearch(x, y, "img/Icon_Surface.png", 100, 8, 16, 602, 151, 635)) ;再次出擊
								{
									Random, x, 1060, 1221
									Random, y, 658, 692
									C_Click(x, y)
...
CloseEventList()
{
	if (GdipImageSearch(x, y, "img/Icon_Events.png", 100, 8, 119, 47, 228, 84))
	{
		LogShow("關閉活動總覽")
		C_Click(1240,66)
	}
...
SystemNotify()
{
	if (GdipImageSearch(x, y, "img/Icon_Announce.png", 100, 8, 64, 87, 331, 120))
	{
		LogShow("出現系統公告，不再顯示")
		if !(GdipImageSearch(x, y, "img/Icon_Announce_Checked.png", 100, 8, 971, 90, 1015, 130))
		{
			C_Click(991, 108)
		}
		C_Click(1193, 103)
	...
	if (AutoLogin)
	{
		If (Find(x, y, 589, 435, 689, 495, Login_In)) ;斷線的登入頁面(密碼登入)
		{
			...
			Loop
			{
				...
				else if (GdipImageSearch(x, y, "img/Icon_CRIWARE.png", 100, 8, 1219, 694, 1267, 727)) ;位於首頁
				{
					C_Click(587, 377)
					sleep 500
				}
		...
		else if (GdipImageSearch(x, y, "img/Icon_CRIWARE.png", 100, 8, 1219, 694, 1267, 727)) ;登入伺服器選擇頁面
		{
...
Capture2(x1, y1, x2, y2, grays=0) {
	...
	if (grays)
	{
		...
		Width := Gdip_GetImageWidth(pBitmap_part), Height := Gdip_GetImageHeight(pBitmap_part) 
		G1 := Gdip_GraphicsFromImage(pBitmap_part)
		Gdip_DrawImage(G1, pBitmap_part, 0, 0, Width, Height, 0, 0, Width, Height, Matrix)
...
GdipImageSearch(byref x, byref y, imagePath = "img/picturehere.png",  Variation=100, direction = 1, x1=0, y1=0, x2=0, y2=0) {
	...
	if (A_TickCount-pBitmapTime>500) or !pBitmapTime ;Get pBitmap every 500ms to save CPU load
	{
		if pBitmap
			Gdip_DisposeImage(pBitmap)
		pBitmap := Gdip_BitmapFromHWND(UniqueID)
		pBitmapTime := A_TickCount
	}
    ...
    if (Gdip_ImageSearch(pBitmap, bmpNeedle, LIST, x1, y1, x2, y2, Variation, , direction, 1)) {
		...
		pw := Gdip_GetImageWidth(bmpNeedle), ph := Gdip_GetImageHeight(bmpNeedle)
		x := Round(ListArray[1]+pw/2, 0), y := Round(ListArray[2]+ph/2, 0)
		Gdip_DisposeImage(pBitmap) ; if it found the image, disposeimage.
		pBitmapTime := 0
	 }
...
SelectMode() {
	...
	if (AnchorMode="停用")
	{
		...
		Loop, 30
		{
			if (GdipImageSearch(x, y, "img/Icon_Attack.png", 100, 8, 117, 48, 212, 81)) ;如果還在出擊頁面
			{
				C_Click(1228, 68)
			}
			else if (GdipImageSearch(x, y, "img/Button_Main_Battle.png", 100, 8, 1017, 389, 1138, 451)) ;成功回到首頁
			{
				Break
			}
			sleep 350
	...
	{
		if (GdipImageSearch(x, y, "img/Icon_HardMode.png", 100, 8, 30, 685, 145, 731)) ;一般關卡的困難 OR 活動難度的困難
		{
			;不做任何事
		}
		else if (GdipImageSearch(x, y, "img/Icon_NormalMode.png", 100, 8, 40, 686, 149, 726))
		{
			C_Click(99,703)
			sleep 1000
			if !(GdipImageSearch(x, y, "img/Icon_HardMode.png", 100, 8, 30, 685, 145, 731))
			{
	...
	{
		if (GdipImageSearch(x, y, "img/Icon_NormalMode.png", 100, 8, 40, 686, 149, 726))
		{
			;不做任何事
		}
		else if (GdipImageSearch(x, y, "img/Icon_HardMode.png", 100, 8, 30, 685, 145, 731))
		{
			C_Click(99,703)
			sleep 1000
			if !(GdipImageSearch(x, y, "img/Icon_NormalMode.png", 100, 8, 40, 686, 149, 726))
	...
	for k, v in imagearray
	{
		...
		if Gdip_ImageSearch(pBitmap, bmpNeedle, List, x1, y1, x2, y2, Variation, , direction, 0, "|", ",")
		{
			...
			Loop, parse, wfp, "｜"
			{
				...
				else if (GdipImageSearch(nx, ny, "img/Unable_Way.png", 100, 8, 422, 348, 439, 374))
				{
...
GetScreen() { ;Get pBitmap for imagesearch
	global
	if pBitmap 
		Gdip_DisposeImage(pBitmap)
	return	pBitmap := Gdip_BitmapFromHWND(WinExist(title))
}
...
;~ v := 40
;~ if GdipImageSearch(x, y, "img/Icon_CRIWARE.png", 100, 8, 1219, 694, 1267, 727) ;(FindFleet(x, y,fleet , 40, 5, MapX1, MapY1, MapX2, MapY2))
;~ {
	;~ WinActivate, %title%
	;~ mousemove, %x%, %y%
